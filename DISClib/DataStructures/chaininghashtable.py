...
def putByPosition(map, key, value, position):
    ...
    """
    try:
        bucket = lt.getElement(map['table'], position)
        entry = me.newMapEntry(key, value)
        ...
        error.reraise(exp, 'Chain:putByPosition')
...
def size(map):
    """  Retorna el tamaNo (número de tuplas) en la tabla de hash.
    Args:
        map: El map
    Returns:
        Tamaño del map
...
def capacity(map):
    ...
    try:
        ...
        error.reraise(exp, 'Chain:capacity')
