...
def addFirst(lst, element):
    ...
    try:
        ...
        if (lst['size'] == 0):
            lst['last'] = new_node
            # lst['first'] = new_node              # CORRECCION Borrar
        else:
            new_node['next'] = lst['first']
            lst['first']['prev'] = new_node        # CORRECCION
            # lst['first'] = new_node            # CORRECCION Borrar

        lst['first'] = new_node                  # CORRECCION instruccion comun al final de if y else 
        lst['size'] += 1
        return lst
...
def deleteElement(lst, pos):
    ...
    try:
        if (lst['size'] == 1) and (pos == 1):
            lst['first'] = None
            lst['last'] = None
            lst['size'] = 0         # CORRECCION Agregar
            return lst              # CORRECCION Agregar
        ...
        if (prev is not None):
            prev['next'] = sig
        else:                                # CORRECCION Agregar
            lst['first'] = sig      # CORRECCION Agregar
            if lst['first'] is not None:     # CORRECCION Agregar
                lst['first']['prev'] = None  # CORRECCION Agregar

        if (sig is not None):
            sig['prev'] = prev
        else:                                # CORRECCION Agregar
            lst['last'] = prev      # CORRECCION Agregar
            if lst['last'] is not None:     # CORRECCION Agregar
                lst['last']['next'] = None  # CORRECCION Agregar

        # if(pos == lst['size']):           # CORRECCION Borrar
        #    lst['last'] = prev             # CORRECCION Borrar
