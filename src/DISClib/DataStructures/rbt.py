...
def put(rbt, key, value):
    ...
    try:
        ...
        error.reraise(exp, 'RBT:Put')

def put_by_position(rbt, key, value, color):
    """
    NEW FUNCTION
    ...
    """
    try:
        rbt['root'] = insertNode_by_position(rbt['root'], key, value, rbt['cmpfunction'], color)
        # rbt['root']['color'] = node.BLACK - NO se requiere
        return rbt
    except Exception as exp:
        error.reraise(exp, 'RBT:Put_by_position')
...
def flipNodeColor(rbnode):
    ...
    try:
        ...
        error.reraise(exp, 'RBT:flipNodeColor')
...
def insertNode_by_position(root, key, value, comparefunction, color):
    """
    NEW FUNCTION
    Ingresa una pareja llave,valor. Si la llave ya existe,
    se reemplaza el valor.
    ...
    """
    try:
        if root is None:     # Se trata de la raíz del árbol y llega con el color apropiado
            root = node.newNode(key, value, 1, color)
            return root

        cmp = comparefunction(key, root['key'])

        if (cmp < 0):     # La llave a insertar es menor que la raiz
            root['left'] = insertNode_by_position(root['left'],  key, value,
                                      comparefunction, color)
        elif (cmp > 0):    # La llave a insertar es mayor que la raíz
            root['right'] = insertNode_by_position(root['right'], key, value,
                                       comparefunction, color)
        else:              # La llave ya se encuentra en la tabla
            root['value'] = value

        # NO aplica reglas de balanceo porque el nodo queda ubicado en la posicion adecuada con su color apropiado
        root['size'] = sizeTree(root['left']) + sizeTree(root['right']) + 1
...
def deleteMaxTree(root):
    ...
    try:
        ...
        error.reraise(exp, 'RBT:deleteMaxTree')
...
def moveRedRight(root):
    ...
    try:
        ...
        error.reraise(exp, 'RBT:moveRedRight')
