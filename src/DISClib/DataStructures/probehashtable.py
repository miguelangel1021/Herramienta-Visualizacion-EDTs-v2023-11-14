...
def put(map, key, value):
    ...
    try:
        ...
        if (pos < 0):           # Se agrega una nueva tupla
            map['size'] += 1
...
def putByPosition(map, key, value, position):
    """ NEW Function
    Ingresa una pareja llave,valor a la tabla de hash en una posicion dada.

    Args:
        map: El map a donde se guarda la pareja
        key: la llave asociada a la pareja
        value: el valor asociado a la pareja
        position: posicion en la tabla de hash. 1 <= position <= map['capacity']
    Returns:
        El map
    Raises:
        Exception
    """
    try:
        #validacion opciones casillas disponibles pero en el pasado ocupadas
        if key == "EMPTY" or key == "Empty" or key == "empty":
            key = '__EMPTY__'
            value = '__EMPTY__'
        entry = me.newMapEntry(key, value)
        lt.changeInfo(map['table'], position, entry)
        if key != None and key != '__EMPTY__' and key != "EMPTY":
            map['size'] += 1
            map['currentfactor'] = map['size'] / map['capacity']
            # CORRECCION
            if (map['currentfactor'] >= map['limitfactor']):
        ...
        error.reraise(exp, 'Probe:putByPosition')
...
def capacity(map):
    """
    NEW Function
    Retorna  la capacidad en la tabla de hash.
    Args:
        map: El map
    Returns:
        Capacidad del map
    ...
    try:
        ...
        error.reraise(exp, 'Probe:capacity')
