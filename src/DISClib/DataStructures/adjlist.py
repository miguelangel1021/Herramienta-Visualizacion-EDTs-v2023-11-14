...
def newGraph(size, cmpfunction, directed, type, datastructure):
    ...
    Args:
        ...
        type: representacion "ADJ_LIST"
        datastructure: archivo Python que implementa la estructura de datos
    Returns:
...
def edges(graph):
    ...
    Para un Grafo No Dirigido, se retorna un unico arco que conecte dos vertices
        ...
        for lstedge in lt.iterator(lstmap):
            for edge in lt.iterator(lstedge):
                if (graph['directed']):
                    ...
                    '''
                    # VERSION ORIGINAL DISClib
                    if not lt.isPresent(lstresp, edge):
                        lt.addLast(lstresp, edge)                
                    '''
                    # CORRECCION DISClib
...
def addEdge(graph, vertexa, vertexb, weight=0):
    """
    Agrega un arco al vertice vertexa entre los vertices vertexa -> vertexb, con peso weight.
    Adicionalmente, si el grafo es No Dirigido se agrega el arco vertexb -> vertexa 
    al vertice vertexb
